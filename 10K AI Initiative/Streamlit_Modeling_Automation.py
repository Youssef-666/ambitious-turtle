...
if choice == "Preprocessing Data" :
    ...
    if True:
        ...
        encoder_columns = (df.select_dtypes(include=['number']).columns).to_list()
        encoder_type = st.selectbox('Imputation Method For Numerical Features', ['','Mean','Median','Mode'])
    ...
    if True:
        ...
        encoder_columns = (df.select_dtypes(include=['object']).columns).to_list()
        encoder_type = st.selectbox('Imputation Method For Categorical Features', ['','Most frequent', 'Put Additional Class'])
        ...
        if encoder_type == 'Put Additional Class' :
            fill_value = st.text_input("Write a filling value for the categorical nulls")
            df[encoder_columns] = df[encoder_columns].fillna(fill_value)
            st.success('Categorical columns filled')
            st.dataframe(df)
    ...
    if True :
        ...
        encoder_columns = st.multiselect('Columns to encode (If your target is numerical select all columns, else select all but your target variable, )',df.select_dtypes(include=['object']).columns)
        # encoder_columns = (df.select_dtypes(include=['object']).columns).to_list()
...
if choice == "Modeling Phase":
    ...
    st.subheader('This is a ' + task_type + ' Problem')
        ...
        if modeling_choice == 'Auto modeling':
            ...
            try:
                if st.button('Go Model'):
                    best_model = compare_models(include=['lr','dt','knn'])
                    compare_df = pull()
